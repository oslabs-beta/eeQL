...
const FileTree = () => {
  ...
  const { userPath, fileTree, activeFileHandler }: any = useContext(
    StateContext
  );
  ...
  const viewHandler = (fileName: string): void => {
    folderView[fileName]
      ? setFolderView({ ...folderView, [fileName]: false })
